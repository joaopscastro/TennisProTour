...
- **UPDATE, later pass — half-closed, not fully:** at the time this status section was first written, every generated/claimed/custom player started at a single fixed age (18 years — solidly senior), meaning no real acquisition flow could EVER produce a junior-eligible player at all; the only way to demonstrate the junior circuit was constructing a `Player` directly via `Player.hire()`, bypassing every real use case. A later pass gave `PlayerGenerationPolicy.generate()` a real `AgeRange` parameter and set the talent pool's/custom-player's shared range (`TALENT_POOL_AGE_RANGE`) to 14-16 years old, so every player now claimed through `ClaimTalentPoolCandidateUseCase` or created via `CreateCustomPlayerUseCase` genuinely starts junior-eligible — proven end-to-end by `apps/api/src/scripts/seedJuniorCircuitWalkthrough.ts`, which now claims every player through the real claim path, no `Player.hire()` shortcut anywhere in it. But 14 years old is EXACTLY the U14/U16 boundary (14×52 = 728 weeks), so every real player starts U16-band-eligible, never U14-band-eligible — U14 remains unreachable by any acquisition path today. See CLAUDE.md's "Player acquisition" section for the full detail (age-scaled scouting noise, the population math behind the batch size, and two more pre-existing bugs found while re-verifying this).
