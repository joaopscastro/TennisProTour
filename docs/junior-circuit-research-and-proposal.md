...
- **UPDATE, later pass — now closed:** at the time this status section was first written, every generated/claimed/custom player started at a single fixed age (18 years — solidly senior), meaning no real acquisition flow could EVER produce a junior-eligible player at all; the only way to demonstrate the junior circuit was constructing a `Player` directly via `Player.hire()`, bypassing every real use case. A later pass gave `PlayerGenerationPolicy.generate()` a real `AgeRange` parameter and set the talent pool's/custom-player's shared range (`TALENT_POOL_AGE_RANGE`) to 14-16 years old, so every player now claimed through `ClaimTalentPoolCandidateUseCase` or created via `CreateCustomPlayerUseCase` genuinely starts junior-eligible. That pass also introduced a second, real bug (found and fixed in the very next pass, not left disclosed-and-unfixed): 14 years old is EXACTLY the U14/U16 boundary (14×52 = 728 weeks) in `RankingBand.juniorEligibilityForAge`, and that function's boundary check used strict `<`, so an exactly-14.000-year-old fell through to U16 — no generated player could EVER land in U14, regardless of the age range. Real Tennis Europe eligibility is inclusive ("14-and-under"), so the check is now `<=` on both boundaries; the fix was to the boundary check, not the age range. A real player is now provably U14-eligible when claimed at exactly the pool's minimum age — proven both by a unit test (`RankingBand.test.ts`) and end-to-end by `apps/api/src/scripts/seedJuniorCircuitWalkthrough.ts`, which claims every player through the real claim path (no `Player.hire()` shortcut anywhere in it) and deliberately claims one player at exactly that boundary age to demonstrate U14 reachability, not just U16. Practically, U14 stays rare in the weekly batch (only that one exact integer week out of the range qualifies, ~1 in 206 generations) — reachable, not abundant; a deliberate range choice, not a further bug. See CLAUDE.md's "Player acquisition" section for the full detail (age-scaled scouting noise, the population math behind the batch size, and two more pre-existing bugs found while re-verifying this).
