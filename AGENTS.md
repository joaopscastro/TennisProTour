...
**Issue 3 (SCALABILITY) — the day tick reloaded every started tournament ever.** New `TournamentRepository.findStartedLive()` returns only started tournaments that can still have work — at least one undecided singles/doubles match, OR a complete qualifying draw whose main draw isn't seeded yet (awaiting promotion). Fully-decided events drop out permanently, so the sweep's cost stops growing with history (the soak's 40s→230s per-tick growth). Used by `SimulateDueMatchesUseCase`, `PromoteQualifiersUseCase`, and `PromoteDoublesQualifiersUseCase`; `findStarted()` stays UNBOUNDED for `ApplyObligatoryTournamentZerosUseCase`, which legitimately needs every decided event in the 52-week window. The SQL is documented as the DB-level form of the aggregate's "not finished" predicate, and the route's private `isSinglesFinished` was extracted to `Tournament.isMainDrawFinished()` (plus `isDoublesMainDrawFinished()`) so there is one definition. Tests: a real-Postgres case proving a fully-finished tournament is excluded while a live one is kept (and `findStarted()` still returns both), plus a unit case proving the finished one is never handed to the sweep.
