...
6. **Notifications** — push/email digest, decoupled via events. 🟢 **STAGE 1 built (the manager results digest)**: `OutboundEmail`/`NotificationPort`/`ManagerContactPort` plus a race-safe `NotificationDeliveryRepository` (the delivery ledger) and `NotificationPreferenceRepository` are real ports; `DrizzleNotificationDeliveryRepository`/`DrizzleNotificationPreferenceRepository`/`DrizzleManagerDigestQuery` are real adapters; `buildManagerDigest`/`renderDigestEmail` (pure) + `SendManagerDigestsUseCase` compute and send. **Deliberately NOT wired into the frozen `advance-world-day` handler yet** (see the update below) — the scheduler wiring is a later stage, and there is no third-party email dependency or notification UI yet.
