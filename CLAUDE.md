...
resistance), and — the actual fix that session — RECOVERS
`FATIGUE_RECOVERY_PER_DAY` on EVERY advanced day in
`AdvanceWorldWeekUseCase` (`recoverDailyFatigue()` mid-week +
`recoverFatigue()` on rollover). Before recovery was wired at all, every
active player trended monotonically to a permanent 100. **The fatigue/form
pass then retuned this from 5 to 3/day** (`docs/balance-tuning-report.md`'s
"Fatigue/form pass" section): at 5/day a week gave back 35, while the whole
senior tour is capped at one tournament/week (5 matches for a 32-draw
...
**Update — onboarding is now possible: a brand-new manager is granted starter XP on account creation (was: 0 XP, permanently unable to sign a first player).** `EnsureManagerAccountUseCase` now credits `STARTER_XP_BALANCE` (500, PLACEHOLDER) the moment an account is genuinely CREATED. Before this, XP was only ever granted from match results and every talent-pool claim costs ≥50 XP (`TalentClaimPricingPolicy`), so a fresh signup had literally no path to their first player — invisible in dev because the seed/bot scripts fund everyone. The grant fires only on real creation (every existing/deleted/suspended path returns before it), so a returning manager never re-gets it. 500 is enough to sign one or two free agents (youngest ~50 XP, a strong prime-age prospect ~150-250) without buying an elite roster outright — a start, not a leg-up (principle #1). **The first version of this grant WAS a check-then-act race, and the earlier "could double it; a few hundred XP is a negligible, self-correcting edge, not worth a transactional insert" characterisation of it here was WRONG — corrected.** A brand-new manager's first page load fires several parallel manager-scoped requests; each missed the `findByAuthSubject` lookup, saved, and credited again (reproduced directly: 8 concurrent first-requests to one new identity returned `500, 2500, 3000, 1500, 3000, 3000, 0, 1000`), so a new manager could start with up to ~6× the intended balance — a real violation of principle #1 (a granted advantage must never unconditionally boost competitiveness) — and a concurrent read could transiently see **0**, rendering "0 XP / Sign disabled". The grant is now atomic and idempotent: `ManagerAccountCreationPort.createWithStarterXp` inserts the account AND its starter-progression row in ONE DB transaction, gated on a conditional `INSERT ... ON CONFLICT DO NOTHING RETURNING` (`DrizzleManagerAccountCreationAdapter`), so exactly one concurrent caller is the creator and therefore the only one that grants — and the account is never visible before its opening balance is, which closes the transient-0 window too. `STARTER_XP_BALANCE` and "grant only on genuine creation" are unchanged. Covered by `EnsureManagerAccountUseCase.test.ts` (grant on create, no re-grant on return, no grant to a suspended/deleted account) plus a concurrent `Promise.all` test against real Postgres at BOTH the adapter level (`DrizzleRepositories.integration.test.ts`: exactly one of 8 concurrent creates grants; balance exactly 500) and through the real HTTP route (`api.integration.test.ts`: 8 concurrent first-requests all report 500).

**Update — race-safety pass, part 1: the two write paths fixable with pure application logic are FIXED.**
- **Practice double-spend (fixed).** `RunPracticeSessionUseCase` used to read `recordedOn`, then award XP/fatigue/ladder, then `record` — a double-click or two tabs both passed the read and double-awarded. Now `PracticeSessionRepository.tryRecord(playerId, day)` is an ATOMIC conditional insert (`.onConflictDoNothing().returning()` — true only for the call that actually created the (player, day) row), called BEFORE any award; the losing call throws "already practiced today" and awards nothing. Covered by a concurrent `Promise.allSettled` test asserting exactly one fulfilment and a single ladder credit.
- **Accept-pair invariant (fixed, window narrowed).** `AcceptDoublesPairUseCase` now re-verifies that neither player is in another non-dissolved pair at acceptance time — the create-side check is check-then-act, so the initiator could have formed/accepted another pair between invite and accept. Covered by a test hand-seeding an active pair plus a stale pending invite and asserting the accept is refused.
- **Still open, was disclosed as needing DB-level atomicity — NOW FIXED (part 2), see immediately below.**

**Update — race-safety pass, part 2: all three DB-level races are now FIXED and verified against real Postgres, not just planned.**
- **(1) Player-to-coach conversion is now atomic (was: XP spend, player release, pair dissolve, and coach insert as separate application-layer writes, so a mid-sequence failure could leave XP spent with no coach).** A new `CoachConversionPort` (`apps/api/src/adapters/outbound/DrizzleCoachConversionAdapter.ts`) does all four in ONE `db.transaction`, mirroring `DrizzleTalentClaimAdapter`: debit XP first (conditional `WHERE xp_balance >= cost`), then a conditional `UPDATE players SET manager_id = NULL WHERE id = ? AND manager_id = ?` (only the ownership column changes — `Player.releaseFromManager()` alters nothing else), then dissolve the player's non-dissolved `doubles_pairs`, then insert the coach; any failure throws an internal `ConversionRollback` so Postgres undoes the already-applied XP debit. `ConvertPlayerToCoachUseCase` dropped its `ManagerXpRepository`/`DoublesPairRepository` dependencies (the port owns those writes) and now publishes the `PlayerConvertedToCoach` event itself, since the adapter reconstitutes the coach (emitting no aggregate events). Tests: 4 new `DrizzleRepositories.integration.test.ts` cases (the full four-table conversion; insufficient XP spends nothing; **the XP debit is rolled back when the player turns out not to be owned — the actual rollback proof**; concurrent conversions of the same player charge exactly once), plus the use case's 8 in-memory tests reworked to an `InMemoryCoachConversionPort` fake. **Still disclosed, unchanged:** the coach-CAP check stays app-level check-then-act, so two near-simultaneous conversions by one manager could exceed the cap by one (XP can never be overspent — the conditional debit guarantees it).
- **(2) Tournament writes are now optimistic-concurrency-guarded (was: `DrizzleTournamentRepository.save` deleted+reinserted every child row unconditionally, so two concurrent registrants for the SAME tournament silently dropped one — last-writer-wins).** `tournaments` gained a `version` column (migration `0047_amazing_jazinda.sql`, `DEFAULT 1`, so every pre-existing row reads back at 1) and `Tournament` an opaque `persistenceVersion` (a documented PERSISTENCE concern, not a domain concept — no domain rule ever reads it) with `markPersisted(version)`. `save` now inserts with `version = 1` (`ON CONFLICT DO NOTHING`) for a fresh/unpersisted aggregate, or runs `UPDATE ... SET version = version + 1 WHERE id = ? AND version = ?` for a loaded one, throwing the new `ConcurrentModificationError` (exported from `application/ports`, so both adapter and route can reference it; the existing global error handler already maps any non-404 domain error → a retryable **409**, so no route change was needed) when 0 rows match. The write-back is load-bearing: `StartDueTournamentsUseCase` saves the same instance up to three times (`fillSlots` for the qualifying field, then `fillSlots` for the main draw, then the bracket seed) and `RegisterEntrantUseCase` saves once after `FormDoublesDrawUseCase.form` mutates that same instance — without `markPersisted`, the second save would fail against the token the first just superseded. Verified safe against regressions because every production save flow holds ONE aggregate instance per id: `SimulateDueMatchesUseCase` loads `findStarted()` but only READS those instances, delegating each simulated match to `SimulateMatchUseCase`/`SimulateDoublesMatchUseCase`, which each reload by id. Tests: 2 new integration cases (two stale writers — the first lands, the second throws `ConcurrentModificationError` and the FIRST's entrant survives; the same instance can be saved repeatedly). No unit-test churn: the version check lives only in the Drizzle adapter, so the in-memory repository fakes are untouched.
- **(3) The weekly entry cap is now transactionally guarded (was: `countSameBandEntriesForWeek` was read-then-write, so two concurrent registrations for the SAME player into two DIFFERENT tournaments both passed and "at most 1 per week" could still land 2).** A new `WeeklyEntryGuardPort` (`apps/api/src/adapters/outbound/DrizzleWeeklyEntryGuardAdapter.ts`) takes a transaction-scoped Postgres advisory lock (`pg_advisory_xact_lock`, released automatically on commit/rollback — it can't leak on a crash) keyed to `(player, season, week, band)`, then recounts the player's real SINGLES + DOUBLES tournaments (deduped by tournament, junior/senior split off `tournaments.age_band IS NOT NULL` — the exact split `isJuniorTier` gives, since `Tournament` enforces junior ⟺ ageBand non-null) UNION any in-flight claims, and inserts a row into the new `weekly_entry_claims` table (migration `0048_keen_nextwave.sql`, PK `(player_id, season, week, is_junior, tournament_id)`, FKs to players and tournaments) only when still under the cap. The tournament being registered is EXCLUDED from the count, so a retry — and a duplicate double-click on the same tournament — still passes, while a genuinely different second tournament is refused. Both `RegisterEntrantUseCase` and `RegisterDoublesEntrantUseCase` call it right after the pre-existing (still present) fast pre-check, so the friendly error message and the non-concurrent path are unchanged; the port is an OPTIONAL constructor dependency in both for test compatibility (the in-memory unit tests keep the old behavior; the composition root always passes it). Tests: 4 new integration cases (a second same-band entry refused at cap 1; **concurrent claims for two different tournaments → exactly one succeeds**; re-claiming the same tournament allowed; junior and senior counted independently).
...
6. **Notifications** — the manager results digest. 🟢 **STAGE 2 complete**: the ports (`OutboundEmail`/`NotificationPort`/`ManagerContactPort`), the race-safe delivery ledger + cursor (`NotificationDeliveryRepository`/`NotificationPreferenceRepository`), the Drizzle adapters, the pure `buildManagerDigest`/`renderDigestEmail`, and `SendManagerDigestsUseCase`; delivery runs on its OWN `notifications` BullMQ queue (explicitly NOT the frozen `advance-world-day` handler), email **fails OFF** (`NOTIFICATION_EMAIL_MODE` defaults to `off`), contact resolution is on-demand via Clerk, and the digest is **default ON with one-click unsubscribe**. See the STAGE 2 update below for the full shape and the explicit non-goals.
...
**Update — Notifications STAGE 1 (the manager results digest) is built: ports, adapters, pure digest logic and the send use case, with the frozen day-tick handler deliberately untouched.** The ledger is two tables (migration `0050_vengeful_fantastic_four.sql`): `manager_notification_states` (one row per manager, `digest_opt_out` default false — ABSENCE OF A ROW MEANS OPTED IN, the product decision is default ON) and `notification_deliveries` (composite PK `(manager_id, kind, window_key)` — the structural "at most one results digest per manager per window" guard; `covered_until` is the cursor; `status` moves `sending` -> `sent`|`failed`). Ports added to `ports.ts`: `OutboundEmail` / `NotificationPort` / `ManagerContactPort` (returns null, never throws, for an unknown manager) and `NotificationDeliveryRepository` (`tryClaim` is an atomic `.onConflictDoNothing().returning()` conditional insert mirroring `DrizzlePracticeSessionRepository.tryRecord`; `previousCoveredUntil` reads ONLY sent rows, so a failed send never advances the cursor) plus `NotificationPreferenceRepository` (default ON). `SendManagerDigestsUseCase` runs per manager: resume from the last SENT cursor (or a 24h `FIRST_DIGEST_LOOKBACK_MS` first-ever window), load and assemble the digest, and SKIP WITHOUT CLAIMING on empty digest / no email / opted-out / already-claimed; a send failure calls `markFailed` and logs, never throwing out of the per-manager loop. Digest content is singles main-draw only: aired results in the window, titles, each player's current band-scoped rank/points (resolved via the same `Record<RankingBand, RankPositionQuery>` pattern `StartDueTournamentsUseCase` uses, band picked by `juniorEligibilityForAge(seasonAgeAnchorWeeks)` — no stored snapshot), and each player's next pending match. `DrizzleManagerDigestQuery` keys windowing EXCLUSIVELY on `scheduled_start_at + coalesce(reveal_seconds, 0)` — `DrizzleTournamentRepository.save` delete/re-inserts match rows so their `created_at`/`updated_at` are useless for "when did this result happen". Two deviations from the brief, both disclosed: (1) the query interface also exposes `listManagerIds()` because no manager-listing dependency existed (the query that already knows rosters owns "who has a digest"); (2) `previousCoveredUntil` filters `status='sent'` (the brief named only the method) — required for the cursor-not-advance-on-failure property. Dev/tests never send real email (the `NotificationPort` is implemented only by a recording fake in tests; no transport adapter and no new npm dependency were added). Tests: `managerDigest.test.ts` (pure round labels / window boundary / assembly / render), `SendManagerDigestsUseCase.test.ts` (opted-out, no-email-not-claimed, happy path, same-window no-op, failure marks failed, empty digest not claimed, 24h first window, multi-day gap covered once, per-manager tolerance), and real-Postgres cases for the two repositories plus `DrizzleManagerDigestQuery` (boundary / out-of-window / qualifying-draw / undecided exclusions). Domain 375, application 248, api 100, worker 8 — all green; web typecheck and full monorepo `tsc --build --force` clean.

**Update — Notifications STAGE 2 is built: the digest is now actually DELIVERABLE end to end, on its own queue, with a real email mode switch and a manager-facing unsubscribe.** Stage 1 built the port/ledger/use-case; this stage wires the outside world to it and closes the loop. **The frozen `advance-world-day` handler in `apps/worker/src/jobs/handlers.ts` was deliberately NOT touched** — the digest runs on a SEPARATE `notifications` BullMQ queue (`apps/worker/src/jobs/notificationJobs.ts`'s `makeSendManagerDigestsHandler`, mirroring `makeAdvanceWorldHandler`'s thin parse/call/log shape), scheduled by its own `upsertJobScheduler('send-manager-digests', { every: NOTIFICATION_DIGEST_INTERVAL_MS })` (default `86_400_000`, PLACEHOLDER) and only registered when the mode is not `off`. That separation is load-bearing: a digest send is a slow external HTTP call and must never be able to stall or fail the world tick. The handler derives `windowKey = isoDayTickKey(new Date())` (the same real-UTC-day helper the day tick uses) and reads `now` ONCE, so the ledger's composite PK makes "one digest per manager per day" structural — a retry or an overlapping firing on the same day claims nothing and sends nothing. **Adapters:** `LoggingNotificationAdapter` (the `log` mode — logs `{ to, subject }` and contacts nothing, mirroring `LoggingEventPublisher`; used by tests and local `log`), the real `ResendEmailAdapter` (`POST https://api.resend.com/emails` over the platform global `fetch`, so NO new npm dependency; throws on non-2xx so the use case marks the delivery `failed` and the cursor — read back only from SENT rows — leaves the window to be re-covered next run), `ClerkManagerContactAdapter` (resolves a manager's email ON DEMAND from Clerk via `createClerkClient({ secretKey })` + `user.primaryEmailAddress`, never stored locally so an address change is picked up on the very next digest; a `dev:` `authSubject` resolves to null, and ANY Clerk error returns null rather than throwing — a transient lookup failure must be a skip, not a wrongly-consumed failed-send window), and `NullManagerContactAdapter` (always null, for `off`/`log`). `composition.ts` resolves `NOTIFICATION_EMAIL_MODE` and wires the pair, exposes `notificationEmailMode`/`notificationDeliveries`/`notificationPreferences`/`notificationContacts`/`notificationDigest` on `Dependencies`, and boot-logs the resolved mode in both processes; `apps/api/src/index.ts` throws at boot if mode is `resend` and `RESEND_API_KEY` is unset (the same loud-misconfiguration style as the `CLERK_SECRET_KEY` check). **The mode fails OFF, not closed** — unset/empty means `off`, which registers no scheduler and sends nothing; a half-wired deploy stays silent rather than blasting mail (the deliberate opposite direction from `AUTH_MODE`'s fail-closed). **Unsubscribe:** `apps/api/src/notifications/unsubscribeToken.ts` signs `base64url(managerId).HMAC-SHA256(payload, NOTIFICATION_UNSUBSCRIBE_SECRET)` (timing-safe verify, returns the id or null, never throws); the footer is appended in the RESEND ADAPTER, not by the application-layer renderer, because the link needs the signing secret + public base URL and is recipient-specific — hence the additive optional `OutboundEmail.managerId` the use case now passes (every existing channel/fake ignores it). `GET /notifications/unsubscribe?token=` is necessarily unauthenticated (it's clicked from email) but unforgeable without the secret and covered by the existing global rate limiter; it 404s when the secret is unset OR the token is invalid, and returns a small HTML confirmation. `GET`/`PUT /me/notification-preferences` back the ONE toggle on the Manager Pro page (no new nav item, no preferences centre), and default is ON (absence of a `manager_notification_states` row = opted in). **Explicit non-goals** (unchanged scope): push/other channels, an in-app notification inbox, a full preferences centre, marketing/campaign email, doubles results in the digest, ranking-movement diffs, and bounce/complaint handling. Tests: real-Postgres end-to-end `SendManagerDigestsUseCase` through the logging adapter (asserts the `results_digest` claim row is `sent`, and a second same-day run sends nothing), `ResendEmailAdapter` with `global.fetch` stubbed (URL/headers/body + footer present/omitted, no network), `ClerkManagerContactAdapter` with a faked directory (primary email; `dev:` -> null; thrown error -> null), and `unsubscribeToken` sign/verify round-trip + tamper/malformed rejection. api suite 117 (was 100 — +16 unit, +1 integration); domain 375, application 248, worker 8 unchanged; full monorepo `tsc --build --force` and `apps/web` typecheck clean.
...
**Update — ranking-based tier entry restrictions (a new, deliberate, owner-requested product rule that mirrors real ATP/ITF entry restrictions).** A player whose SENIOR ranking is too good may no longer drop into a lower senior tier: a top-`FUTURES_MAX_RANK` (200) player may not enter a `futures` event, and a top-`CHALLENGER_MAX_RANK` (50) player may not enter a `challenger` event. `tour` and `major` are unrestricted, every junior tier is unrestricted (this is a senior-tour rule; the junior ladder has its own age-based eligibility rule), and an UNRANKED player ("NR" — no qualifying result in the senior band) is never blocked, since with no earned ranking there is nothing to be "too good" for. The rationale, recorded on the policy itself: it stops a strong player farming weak tiers, and it makes "which tournament should I enter?" a real decision because a player's eligible list now narrows meaningfully as their rank rises. **The rule lives in one new domain module, `packages/domain/src/ranking/TierEntryRestrictionPolicy.ts`** — `FUTURES_MAX_RANK` and `CHALLENGER_MAX_RANK` are explicit PLACEHOLDER constants (flagged the same way aging thresholds, `DIRECT_ACCEPTANCE_CUTOFF` and the ranking point tables are; owned by the balance-tuning pass, not sourced), held in a `MAX_SENIOR_RANK_BY_TIER` map so a future restricted tier is added without touching a caller, with `maxSeniorRankForTier`, the pure predicate `isRankTooHighForTier(tier, rank)` and the one shared reason-builder `seniorTierEntryRestrictionReason(tier, rank)`. **Enforcement** is in both registration use cases, using the SAME live senior `RankPositionQuery` the qualifying/DA rule already reads (and only reading it at a restricted tier, so an ordinary tour/major/junior entry pays no extra ranking cost): `RegisterEntrantUseCase` throws `Player <id> is ranked #<n> on the senior ladder — too high to enter a <tier> event`, and `RegisterDoublesEntrantUseCase` gained an optional `seniorRankPosition` (composition always passes it) and throws the identical message, so doubles can never be the loophole around a refused singles entry. Both are optional dependencies, so the pre-existing in-memory unit tests where the rule is simply inert keep working. **Surfaced** in the player-scoped open-tournament preview (`GET /tournaments?status=open&playerId=`, `attachEntryInfo` in `tournamentRoutes.ts`): each row gains `rankRestricted: boolean` + `rankRestrictedReason: string | null`, computed from the SAME `seniorTierEntryRestrictionReason` the use cases enforce, so the disabled row and the server refusal can never disagree. The frontend (`PickableTournament`, `TournamentDto`) carries both fields; `tournamentRefusalReason` returns the server's reason so `EnterTournamentModal`, the planner's `WeekRegisterPicker` and `SinglesEntryPanel` all disable/mark the row with the real explanation, and the picker's default "Eligible to enter" circuit filter now also drops rank-restricted senior events (it is a genuine permission rule, matching the label). Tests: a new `TierEntryRestrictionPolicy.test.ts` (both cutoffs, just-outside boundaries, unranked-never-blocked, junior-never-restricted, the exact reason), five new `RegisterEntrantUseCase.test.ts` cases (top-200 refused from futures but allowed challenger/tour; top-50 refused from challenger but allowed tour; 201 allowed in futures and 51 in challenger; unranked allowed everywhere; the same rule on the doubles path) and a real-Postgres `api.integration.test.ts` case proving the preview flag and the POST enforcement are the same decision (a real rank-100 player shows `rankRestricted: true` for futures / `false` for challenger, the futures POST 409s with the reason, the challenger POST 201s). Domain 386 (was 380), application 268 (was 263), api 147 (was 146), worker 11 — all green; full monorepo `tsc --build --force` and `apps/web` typecheck clean. The worker tick handler was deliberately NOT touched.
...
1. **Notifications (bounded context #6)** — STAGE 1 (the manager
   results digest) is built: ports, adapters, pure digest logic and the
   send use case. What remains is the scheduler wiring (deliberately NOT
   the frozen day-tick handler) and a real email transport adapter.
   **Social (bounded context #7)** — still genuinely unstarted (the
   manager ladder covers "leaderboards"; guilds/academies/chat remain
   entirely unbuilt).
...
3. ~~**Balance/tuning pass (GC-5.2 in the roadmap)** — `effectiveRating`'s
   ...
   **The real, disclosed implication**: "436 entries, zero titles" most
   ...
   `tsc --build --force` clean.
   **Update — the fatigue/form half of this item (the docs' repeatedly
   flagged "main open balance question") is now measured and partly
   retuned, per `docs/balance-tuning-report.md`'s "Fatigue/form pass"
   section.** Three new buckets in `apps/api/scripts/balance-simulation.mjs`
   (`form`, `formTrajectory`, `fatigueTrajectory`, each replaying the real
   production mutators, plus a `FATIGUE_RECOVERY_PER_DAY` env override for
   candidate comparison) and one production constant changed:
   `FATIGUE_RECOVERY_PER_DAY` 5 → 3. At 5/day a week gave back 35 fatigue
   while the ENTIRE senior tour is capped at one tournament/week (5 matches
   for a 32-draw champion, ~6 each), so even a player who WON a title every
   week sat at fatigue 0 forever — the mechanic was dead on the senior tour;
   3/day moves the accumulation threshold to ~4 matches/week. The form curve,
   `BASE_MATCH_FATIGUE`, the sim penalty, and every other fatigue constant
   were MEASURED and deliberately left unchanged (the `[12,25]` sweet spot
   is genuinely the curve's peak and is reachable under a deep-run schedule).
   One disclosed, unfixed artifact: `decayForm`'s `Math.round` makes 1-3
   fixed points, so an idle player stalls at form 3 instead of decaying to 0
...
## 3-season soak run — three bugs found and fixed

**Bug 1 (HIGH) — doubles draws formed but never played a single match.**
Evidence: `SELECT draw, count(*) FROM tournament_doubles_matches GROUP BY draw;` → `main | 2262 | decided 0`; `doubles_titles` = 0 (a doubles-focused manager made 264 entries and played zero matches). Root cause: `SimulateDueMatchesUseCase.sweepDoubles` chose its draw from the STATIC tier flag (`tournament.hasDoublesQualifying && !tournament.isDoublesQualifyingComplete() ? 'qualifying' : 'main'`); when `FormDoublesDrawUseCase` falls back to seeding the MAIN doubles draw directly (its qualifying field too sparse to seed), the qualifying bracket is genuinely never seeded, so the expression still yielded `'qualifying'` and the very next line (`draw === 'qualifying' && !hasDoublesQualifyingDrawStarted → return`) skipped the formed main draw forever. Fix: qualifying now counts as live ONLY once actually seeded and not complete (`hasDoublesQualifyingDrawStarted && !isDoublesQualifyingComplete()`), otherwise the sweep falls through to the main draw (whose own `!hasDoublesDrawStarted` guard handles "not seeded yet"). Pinned by `SimulateDueMatchesUseCase.test.ts` ("sweeps the doubles MAIN draw when doubles qualifying is configured but was never seeded"). **The singles path had the same latent routing shape** (`hasQualifying && !isQualifyingComplete()`); it was only ever saved from a dead-end because a singles main draw is never seeded before qualifying completes in production, but its selection is now guarded identically (require `hasQualifyingDrawStarted`) so it can't regress.

**Bug 2 (MEDIUM) — retired players permanently occupied roster slots.**
Evidence: `SELECT count(*) FROM players WHERE stage='retired' AND manager_id IS NOT NULL;` → 2; both affected managers sat at cap 2/2 and were refused a replacement claim 271 times. Root cause: the roster-cap check counted every row `findByManager` returned, and retirement deliberately RETAINS `manager_id` (history/lineage). Fix: new `activeRosterCount` (`packages/application/src/use-cases/rosterCap.ts`) excludes retired players and is used by both cap call sites, `ClaimTalentPoolCandidateUseCase` and `CreateCustomPlayerUseCase`. Retirement still retains `manager_id`, and the roster dashboard still LISTS retired players (they stay visible, they're just not counted against the cap). Pinned by `ClaimTalentPoolCandidateUseCase.test.ts` ("does not count a retired roster member against the cap").

**Bug 3 (LOW) — qualifying completed but the main draw could never be seeded.**
Evidence: 3 tournaments (`tour` 64-draw and `challenger` 32-draw) were `has_started=true`, qualifying fully decided, yet had no main-draw matches and no title — permanently stuck. Root cause: after `PromoteQualifiersUseCase` promoted the winners, a resulting main draw too sparse for `BracketGenerator` (for a 16-draw, fewer than 9 entrants means every entrant gets a bye and no round-1 match exists) left `hasMainDraw` false, and the use case was a one-shot no-op thereafter. Fix: when promotion leaves the main draw too sparse, pad it from the eligible fill-only pool (the exact selection `StartDueTournamentsUseCase.fillSlots` already used, extracted to a shared `packages/application/src/use-cases/fillDrawSlots.ts`) and then seed it. Because the tournament has already started, a new domain method `Tournament.addMainDrawFiller` (plus the doubles analogue `addDoublesMainDrawFiller`) adds a manager-less filler after start but before the main bracket is seeded. `PromoteDoublesQualifiersUseCase` gets the same rescue with filler PAIRS. Idempotent by construction: an already-seeded main draw is skipped by the existing `hasMainDraw`/`hasDoublesDrawStarted` guards, and already-promoted winners are filtered before promotion. Pinned by new cases in `PromoteQualifiersUseCase.test.ts`, `PromoteDoublesQualifiersUseCase.test.ts`, and `Tournament.test.ts`. All fixes live in the use cases the day-tick handler already calls — no new worker systems. Domain 375→378, application 248→252, api 118, worker 8; full `tsc --build --force` and `apps/web` typecheck clean.

## Second 3-season soak pass — four structural issues (under-filled draws, stuck draws, tick cost, stale scheduler)

**Issue 1 (MEDIUM) — started draws were under-filled; root cause is floor-vs-demand, and the fix is DELIBERATELY NOT APPLIED pending an owner decision.** 510 started tournaments had fewer entrants than their draw size (`tour` 64-draws with 4). Diagnosis, computed from the schedule policies and the qualifier/wild-card reserved-slot rules rather than guessed: a filler can pad only ONE tournament per week (the weekly-commitment exclusion in `fillDrawSlots`), so the per-band `FILL_ONLY_FLOORS` must cover a whole week's slot DEMAND. Weekly demand: senior 236 (futures 2x32 + challenger 2x43 + tour 1x86), +238 on a `major` week; junior 272 per band on the every-8-week peak (j30 3x16 + j60 2x16 + j100 32 + j200 32 + j300 64 + j500 64), x3 bands (u14/u16/u18) = 816. Total peak = 1052 (1290 on a major+junior-peak week), versus floors summing to 290 (~a quarter). The soak's started-but-short draws are the visible symptom. Because raising the floors to ~1052-1290 exceeds the 1000 threshold at which population growth becomes its own problem (`players.findAll()` runs every day tick), the floors are LEFT UNCHANGED; the demanded number (1052 peak, 1290 with a major) is recorded in `EnsureFillOnlyPopulationUseCase`'s `FILL_ONLY_FLOORS` doc comment for the owner's floors-vs-slate-size decision. A secondary, independent under-fill cause was also found and left for the same pass: a qualifying tournament's main draw is deliberately opened with `mainDrawCapacity = drawSize − qualifierSlots − wildCardSlots`, and its wild-card places are only ever filled by REAL local qualifying registrants (`applyWildCards`), so an event with no local qualifier starts `wildCardSlots` short even when the filler pool is full. **Update — that secondary cause is now FIXED; the floor-vs-demand decision above still stands.** The wild-card reservation was never meant to be permanent: the automatic algorithm fills it only when a host-country qualifying registrant exists, and UNAWARDED places are now treated as fillable. `StartDueTournamentsUseCase` pads the main draw up to `drawSize − qualifierSlots − <wildcards actually awarded>` (counting awarded places via the canonical `Tournament.wildCardSlotsTaken` accessor, shared with the tournament DTO), not the static `mainDrawCapacity`, and `PromoteQualifiersUseCase` tops an auto-started event up to a full draw — so a tournament whose host country matches no qualifying entrant no longer seeds `wildCardSlots` short when the pool is available, while still never exceeding `drawSize`. `FILL_ONLY_FLOORS` and the tournament slate are deliberately UNCHANGED: short draws that result from genuine filler SUPPLY are accepted by design (the owner's floors-vs-slate-size decision above); the wild-card reservation was a structural shortfall, and it is not.

**Issue 2 (MEDIUM) — never-started draws accumulated forever.** `StartDueTournamentsUseCase` now expires never-started, genuinely EMPTY draws (`entrants.length === 0 && doublesEntrants.length === 0`) whose scheduled week is more than `ABANDONED_TOURNAMENT_EXPIRY_WEEKS` (2) in the past, via a new optional `TournamentRepository.deleteAbandonedTournament`. DELETE was chosen over a soft state because the schema has no tournament-status column and these are hollow shells (the child tables cascade; the one non-cascading FK, `weekly_entry_claims`, is deleted first), so keeping them would only accumulate dead rows. The Drizzle adapter re-guards the delete in one transaction (never-started + all six state tables empty), and the pass runs BEFORE the fill/start loop so a shell can never be filled first. Idempotent (a deleted shell never reappears). Tests: four in `StartDueTournamentsUseCase.test.ts` (expires past-threshold empty; leaves an entered draw; leaves a within-threshold draw; never expires one that starts this run) plus a real-Postgres guarded-delete case in `DrizzleRepositories.integration.test.ts`.

**Issue 3 (SCALABILITY) — the day tick reloaded every started tournament ever.** New `TournamentRepository.findStartedLive()` returns only started tournaments that can still have work — at least one undecided singles/doubles match, OR a complete qualifying draw whose main draw isn't seeded yet (awaiting promotion). Fully-decided events drop out permanently, so the sweep's cost stops growing with history (the soak's 40s→230s per-tick growth). Used by `SimulateDueMatchesUseCase`, `PromoteQualifiersUseCase`, and `PromoteDoublesQualifiersUseCase`; `findStarted()` stays UNBOUNDED for its remaining per-request/diagnostic callers (see the next note). The SQL is documented as the DB-level form of the aggregate's "not finished" predicate, and the route's private `isSinglesFinished` was extracted to `Tournament.isMainDrawFinished()` (plus `isDoublesMainDrawFinished()`) so there is one definition. Tests: a real-Postgres case proving a fully-finished tournament is excluded while a live one is kept (and `findStarted()` still returns both), plus a unit case proving the finished one is never handed to the sweep.

**Issue 5 (SCALABILITY) — the weekly obligatory-zero rule was the last per-tick unbounded tournament read; it is now bounded to its ranking window.** `ApplyObligatoryTournamentZerosUseCase` ran once per weekly rollover against `TournamentRepository.findStarted()` — every tournament that had ever started, each fully reconstituted (entries + matches). On the same 3-season soak that drove Issue 3 that was **3,706 tournaments / 4.4s**, versus `findStartedLive()`'s 27 tournaments / 0.7s — the main remaining per-tick growth driver. The use case only ever needs obligatory-tier events whose final was decided INSIDE the rolling `RANKING_WINDOW_WEEKS` (52) window, because each mandatory-skip zero is dated to the event's `weekScheduled` and ages out of the window on the real schedule; anything older can never produce a zero. New optional `TournamentRepository.findStartedWithinWindow(currentWeek, weeks)` is the bounded counterpart, implemented in `DrizzleTournamentRepository` as a single SQL prefilter over the same absolute-week arithmetic `weeksBetween` uses (`season * 52 + week`) with an inclusive `BETWEEN` on `[currentWeek - weeks, currentWeek]` — so the expensive per-tournament `load()` (5 child queries each) never runs for an out-of-window event, mirroring how `findStartedLive()` was done. The use case now reads it, falling back to `findStarted()` only for in-memory fakes that don't implement it, and KEEPS its own JS window filter on top, so behaviour for anything inside the window is byte-identical (the 8 existing `ApplyObligatoryTournamentZerosUseCase` tests pass unchanged). `findStarted()` itself is deliberately unchanged; its remaining callers were audited: the `GET /tournaments?status=started` route (a per-REQUEST read, not a per-tick one — it already filters to at most ~3 weeks back, so it could be bounded later, deliberately out of scope here), the bootstrap script's diagnostic started-count, and the test-compat fallbacks in the day-tick use cases when a fake omits `findStartedLive` — none of which is a per-tick growth driver. Tests: a real-Postgres case proving an in-window event is returned fully reconstituted while an out-of-window event and a never-started one are not (and `findStarted()` still returns all started). api 121 (was 120), domain 379, application 260, worker 11 — all green; full `tsc --build --force` and `apps/web` typecheck clean.

**Issue 4 (OPS) — a renamed scheduler left a stale repeatable job in Redis.** `upsertJobScheduler` never removes a schedule whose id a later version drops, so the old `advance-world-week` (and the retired `simulate-due-matches` on the old `matches` queue) kept firing; the still-running world Worker processed the stale payload against a world that no longer existed and logged `job failed`. New `apps/worker/src/jobs/reconcileSchedulers.ts` (`removeLegacySchedulers`) removes a small, hard-coded legacy id list on boot and logs each removal; best-effort (a failure is logged, never fatal). Wired in `apps/worker/src/index.ts` before the current scheduler is registered. Tests: three in `reconcileSchedulers.test.ts` (removes+logs existing, silent no-op when absent, logs rather than throws on failure).

Test counts after this pass: domain 378, application 257 (was 252), api 120 (was 118), worker 11 (was 8); full `tsc --build --force` and `apps/web` typecheck clean.

## First-visit UI pass — replay 404 + bracket counts + Scouting XP (four reported bugs)

**The reported replay 404 was NOT a regression from the per-world match-log change — it was a bracket-display bug.** A visitor reached `/replay/<id>-r3-m3` from a DECIDED-looking round-of-16 card and got "not found", while r1-m0 in the same tournament served. Checked against real Postgres + the on-disk blobs: that tournament (`tour`, 64-draw) had r1 (32) and r2 (16) decided, and **r3 present but 0 of 8 decided** — so no r3 replay blob exists and none should. The per-world store is fine (the API served r1 through `<MATCH_LOG_DIR>/<WORLD_ID>/` exactly as the worker wrote it). The real defect lived in `apps/web/app/tournaments/[id]/page.tsx`: a round was considered "all aired" via `matchAirState`, which returns 'aired' for an UNDECIDED match (there is no reveal window to wait for) — so an entirely un-played generated round was labelled "Decided", collapsed into fake "X def. Y" rows, and, in the collapsed branch, linked to replays that were never written. Extracted the status/collapse rule to a pure, tested `apps/web/lib/bracketStatus.ts` (`roundStatus`/`roundCollapsed`) that only counts a decided match as aired; the page now only wraps a DECIDED match in a replay `<Link>`, so a not-yet-played match is never presented as decided or clickable. (Decided bracket cards were always real, focusable `next/link` anchors that navigate — verified by real clicks; the mislabelling was what made visitors click un-played cards, which correctly do nothing in the expanded layout.) This also fixes the "Decided — 0 of 8 played" contradiction and adds a "Scheduled" badge for "all played, reveal not started yet" instead of calling it "Airing" while every card still reads "Starts in …".

**Counts exceeding the draw size were a real DTO-vs-display bug, not a relabel.** `TournamentDto.entrants` covers BOTH draws (a qualifying-tier event's below-cutoff field sits in `'qualifying'`), so a 64-draw `tour` with a 24-player qualifying field has 88 entries — and the list row and bracket hero displayed `entrants.length` against `drawSize` ("88/64", "44 players" next to "DRAW 32"). `toTournamentDto` now exposes `mainDrawEntrants` (entrants whose `draw` is `'main'`, always ≤ `drawSize`), and the list row / planner card / bracket hero read THAT. A real-Postgres case in `api.integration.test.ts` pins a mixed 16-draw (1 main + 2 qualifying) reading `entrants: 3, mainDrawEntrants: 1`.

**Scouting's "YOUR XP 0" + disabled Sign buttons was a false-zero race.** The page derived `xpBalance = entitlement?.xpBalance ?? 0`, so while the entitlement fetch was in flight it read as a REAL 0 — "YOUR XP 0", every Sign disabled, "Need N more" — even as the sidebar (which only renders once the value is known) showed the true balance. "Not loaded yet" is now its own state: `apps/web/lib/xp.ts`'s `xpAffordability(balance | null, cost)` returns `'unknown' | 'affordable' | 'short'`, the body renders "—" until loaded, and the "Need N more"/disabled gating never treats unknown as 0. Pure cases in `apps/web/e2e/display-logic.spec.ts` pin unknown ≠ 0 and the round-status matrix.

Verified live against the running dev stack (Playwright): the undecided r3 column now reads "Upcoming" with **zero** replay links (was 8 fake ones), and Scouting shows "—" while loading then the true balance with no false "Need N more". Domain 379, application 260, api 123 (was 122), worker 11; full `tsc --build --force` and `apps/web` typecheck clean. **Verdict on the reported 404: the visitor reached a match that was genuinely never played through a display bug — not a broken store, and not a fabricated id.**

## First-session comprehension pass (naive-user walkthrough — 2 findings, both 4/4 agents)

Two frontend-first fixes from four independent LLM agents walking the UI cold. Neither changes game systems: no pool filtering, no claim-pricing change, no ranking-logic change.

**Finding 1 — the Scouting pool called everything a "prospect" and hid that free agents are real, competing, experienced players.** A free agent is a real `Player` who keeps playing while unsigned, so a manager could sign "a prospect" and discover "SENIOR #3, 165 pts, 1 career title, $19K career earnings" with a match already in progress they never entered. The mid-draw adoption was previously disclosed below as a "known, disclosed edge … harmless, just slightly odd" — **that characterisation was wrong**: it was the single most confusing thing in a first session, per all four agents (corrected inline above). Fix, frontend-first + one minimal DTO extension: the Scouting page is honestly labelled **free agents** (hero subhead + info panel), stating the pool spans raw teenagers to established players; every card shows observable career context — `titleCount` and `careerPrizeMoney`, plus a **"Competing · <tournament>"** badge when the player is currently alive in a draw; and BOTH signing points (the Scouting grid and the free-agent banner on the player profile) confirm in plain language that signing adopts them into that event mid-tournament, with the same disclosure repeated in the success notice. DTO fields added to `GET /talent-pool` (all observable/public — nothing hidden): `careerPrizeMoney` (already on the `Player` aggregate), `titleCount` (new `DrizzleTitleRepository.countByPlayers`, one grouped query), and `currentTournament` (new `DrizzlePlayerMatchesQuery.liveTournamentByPlayer` — one batch read over undecided singles/qualifying matches for the whole pool; doubles match rows key by pair id, so a player only in a doubles draw isn't flagged — a disclosed limit). No new backend concept, no pool filtering, veterans stay signable, claim pricing untouched. **Superseded on one point:** three successive labelling attempts (honest copy, the `COMPETING` badge, an explicit confirm dialog) did NOT fix the confusion, because the problem was the RULE, not the label — see the "Free-agent commitment rule" section below. The confirm-adopt flow is gone; committed free agents are now shown but not signable at all.

**Finding 2 — ranks contradicted each other across screens because they were shown without their band.** A player can hold several independent ranks at once (e.g. unranked on the U16 ladder, Senior #3 on the senior ladder — both true), but the roster row showed a bare "#NR" while the profile headlined "SENIOR #3", and the `/rankings` header omitted U18 from its "separate ladders" sentence. Fix, labelling + copy only, no ranking-logic change: every rank now carries its band — the roster rank plate always reads "SENIOR ranking #3" / "U16 ranking #NR", the replay participant pills carry the band they came from (`RankPill`/`VersusPlayer` now render `bandLabel`), and the standings table's Rank column names its ladder. Empty/empty-looking ladders are explained in place: the `/rankings` page states each ladder only counts its own events (a junior who wins a senior event earns Senior points, never junior ones) and repeats that in the empty state, and an unranked roster/profile row now says which ladder it is unranked on and why. The standings header now lists all four bands (Senior, U18, U16, U14). New shared presentation helpers `RANK_BAND_LABEL`/`rankingBandScopeNote` in `apps/web/lib/format.ts`, pinned by pure cases in `apps/web/e2e/display-logic.spec.ts`.
...
## Free-agent commitment rule (deliberate design change)

**A free agent who currently has an UNFINISHED tournament commitment
cannot be signed.** This is a deliberate RULE change, not a label change —
it replaces the old behaviour where signing a mid-draw free agent silently
adopted the new manager into a draw they never entered (with an existing
match, career earnings and even titles already attached). Four rounds of
naive first-time-user walkthroughs found that "sign a prospect, discover
they're SENIOR #3 with a match in progress" confusion to be the single
most persistent problem, and three successive labelling attempts (honest
copy, the `COMPETING` badge, an explicit confirm dialog) did NOT fix it
because the problem was the rule, not the label. The rule is now: a
signing is always clean — the new manager never inherits an in-progress
draw.

- **"Unfinished" = the tournament's MAIN draw has not been played to its
  final.** The single definition is `Tournament.isMainDrawFinished()` /
  `isDoublesMainDrawFinished()` (`packages/domain/src/competition/Tournament.ts`,
  lines ~484 and ~494 — the same predicates the `/tournaments?status=started`
  filter and `findStartedLive()` already use). A tournament that has not
  started, is mid-draw, or is between qualifying and main-draw seeding is
  therefore UNFINISHED; one whose main draw is decided is FINISHED.
  Deliberately TOURNAMENT-level, not player-level: a player eliminated in
  round 1 of an event still in progress remains committed until that event
  concludes. Both the singles (`tournament_entries`) and doubles
  (`tournament_doubles_entrants` registrations AND formed
  `tournament_doubles_pairs`, so a filler placed into a pair is covered)
  entry paths are checked.
- **Enforcement is race-safe, in the atomic claim itself.** The SQL twin
  of the predicate lives in one shared module,
  `apps/api/src/adapters/outbound/unfinishedCommitment.ts`
  (`noUnfinishedCommitment(playerId)`), and is appended to the SAME
  conditional `UPDATE players ... WHERE id = ? AND manager_id IS NULL AND
  <no unfinished commitment>` in `DrizzleTalentClaimAdapter` — not a
  separate pre-check, so two concurrent claims, or a draw that seeds
  between page load and click, can never slip a committed player through.
  A new `TalentClaimOutcome` kind, `player-committed`, distinguishes this
  refusal from `player-unavailable` so `ClaimTalentPoolCandidateUseCase`
  throws a plain-language message ("still has an unfinished tournament —
  they can't be signed until it concludes") rather than a generic failure.
- **The pool SHOWS committed free agents, it just can't sign them.** The
  Scouting card renders a "🔒 In a draw · <tournament>" badge and disables
  Sign with the real reason; the player-profile free-agent banner does the
  same. The DTO carries `signingBlocked: boolean` and
  `blockingCommitment: { id, name } | null`, computed from
  `DrizzlePlayerMatchesQuery.unfinishedCommitmentByPlayer()` — the exact
  same predicate the atomic claim enforces, so the client's disabled
  state and the server's refusal can never disagree. Value-hiding
  discipline is intact (a tournament's id/name is public; nothing hidden
  is exposed). The mid-tournament confirm/adopt flow is removed from both
  signing points, since it can no longer happen.
- **Tradeoff, stated plainly:** the signable pool is temporarily smaller —
  a free agent competing in a draw (or even one eliminated from an event
  that is still running) is unavailable until that tournament concludes.
  That is deliberate, and it is the direct replacement for the previous
  "inherit the draw" behaviour. It also means a filler pulled into a draw
  is temporarily unsignable; the pool is never hidden or filtered, so the
  manager sees the player and the reason.
- **Verified** against real Postgres, not just unit-tested: a committed
  free agent is refused with nothing spent and ownership untouched (both
  the singles and doubles paths), an uncommitted one still signs, and a
  player whose only tournament's main draw has FINISHED becomes signable
  again. A player entered in a not-yet-started draw is flagged too (the
...
## Sixth naive-walkthrough pass — XP single-source + real match-time replay inputs

Two findings from a sixth first-time-user round.

**1. XP is now ONE shared source per browser session, and updates after a spend without a reload.** The reported symptom (sidebar showing 500, then 423 on /rankings, then 500 again) was investigated to its actual cause, and it was NOT a product bug: the walkthrough harness's driver accepts an absolute URL, so the naive agent navigated to a *sibling session's* web server (each ux-probe session inlines its own `NEXT_PUBLIC_DEV_MANAGER_ID`), and the sidebar correctly showed that other manager's true balance — proven by `chrome_debug.log`/Code-Cache entries for `localhost:3005` in agent-3's browser profile at the exact timestamp of the screenshot, and by the DB (`ux-agent-4`'s balance was 423, `ux-agent-3`'s was 500). The panel was faithful; the agent was looking at another manager. Even so, the genuine gap it pointed at was real and is fixed: each page fetched entitlement into its own local state, so a page whose React state survives a client-side navigation could show a pre-spend balance, and signing from a player's profile never refreshed it. New `apps/web/lib/entitlement.ts` is the single source — a module-level store keyed by manager id with `useEntitlement` (subscribes, fetches once, re-reads on focus/visibility/pageshow) and `refreshEntitlement` (one shared in-flight request; a failure keeps the last-known value rather than blanking it). Every XP surface (homepage, Scouting, Rankings, Managers, Manager Pro, player profile) reads it, and both spend points (talent claim, coach conversion) plus the player-profile sign refresh it — so the balance is consistent across screens and current without a hard reload. The store is a thin cache, so it is not unit-tested directly; the web typecheck and the existing pure specs stay green.
...
## Deadlock fix batch — where a 52-week agent-played season permanently trapped the free-agent pool

**The deadlock, verified by SQL on the real agent world.** `tennis_manager_agents` (52 agent-played weeks) sat at 549/549 free agents unsignable for 49 straight weeks, 48 never-started draws (18 with manager entries), 223 started draws short. A week's slate needs ~692 singles filler slots (1,052 peak) while the pool was 549 and one filler can pad only ONE tournament per week; once a week saturated, later draws could never reach `BracketGenerator`'s strict-majority seedable threshold, a never-started draw with any manager-owned entrant was deliberately never expired, and the "no signing while committed" rule then locked every player inside those draws FOREVER. Seven changes fix the root cause and the trap:

- **P2 — `next`-match ordering.** `DrizzlePlayerMatchesQuery.forPlayer`'s unscheduled tie-break sorted tournament week DESCENDING, contradicting the port's "earliest not-yet-simulated" contract. Now season → week → round ASCENDING (scheduled-first ordering unchanged); 3 real-Postgres cases.
- **Harness evidence bug.** `soakEvidence.mjs`'s `cohortDeltas` took `weekly[0]` as the "first" snapshot, but a player claimed later has `rows: []` there, so every id read `present: false` and the `experience-rising-skills-flat` anomaly (the Skill-rounding bug's signature) passed VACUOUSLY in every soak. It now scans every snapshot (first occurrence wins), and `agentSeason.mjs` actually pushes the anomaly (it never ran the check at all). New `apps/api/scripts/lib/soakEvidence.test.mjs`, wired into the api suite.
- **P1-A1 — demand-aware filler supply (root fix).** `EnsureFillOnlyPopulationUseCase` keeps `FILL_ONLY_FLOORS` as a safety net and adds a second pass that sums the ACTUAL fill demand of the week just generated (`mainDrawCapacity + qualifyingDrawSize` per open draw scheduled for it) and sizes each band's pool to `demand × FILL_DEMAND_HEADROOM_FACTOR` (1.3). It counts only AVAILABLE fillers (a committed 273-strong u14 pool used to read "satisfied" while 11 due draws starved) and excludes cancelled draws. Idempotent. Pilot: pool 549 → 1,420 at bootstrap, 1.6-1.7k in play.
- **P1-A2 — scouting pagination.** `GET /talent-pool?limit=&offset=&signableOnly=` (default 64, max 256) returns `{ candidates, total, poolTotal, availableTotal }`; `signableOnly` is applied in SQL via the SAME `noUnfinishedCommitmentFor` predicate the atomic claim enforces (the predicate gained a column-reference form), so a signable page can never contain a row the claim would refuse. `findFreeAgents` pages youngest-first (age, then id); `countFreeAgents` is one grouped `FILTER` count. The Scouting page's "Available only" is now a real server filter, "Show more" pages by offset. Value-hiding untouched.
- **P1-A3 — no cross-week double-booking.** The fill exclusion only knew same-week entries, so a filler alive in a 14-day major could be placed into a later week's draw. New `TournamentRepository.findUnfinishedCommitmentPlayerIds()` (the set form of the signing predicate) is preloaded by `StartDueTournamentsUseCase` and consumed in `fillDrawSlots`, which adds ids as it fills so one run can't reuse a filler either. Absent on in-memory fakes → old behavior.
- **P1-C1/C2 — a TERMINAL cancelled state, and signing unblocked by it.** `Tournament` gained `cancelledAt`/`cancelReason`/`cancel()` (idempotent, never-started-only) with registration/start refusals; persisted via `tournaments.cancelled_at`/`cancel_reason` (migration `0051_empty_thunderball.sql`); `findOpenForRegistration` excludes cancelled. `StartDueTournamentsUseCase` cancels a still-unseedable, never-started draw past `CANCELLED_DRAW_GRACE_WEEKS` (4) — entries KEPT, never deleted. `unfinishedCommitment.ts`'s two fragments now begin `t.cancelled_at IS NULL AND (...)`, so every consumer (atomic claim, pool filter, pool counts, filler set) treats a cancelled draw as concluded and its players become signable again. The empty/filler-only delete path is unchanged.
- **P1-C3 — say it visibly.** `TournamentDto` gained `cancelled`/`cancelReason`; the tournament detail page shows a cancelled banner + reason and a "This draw was cancelled" panel (no entry control); `latestCancelledEntry` drives the roster's "Cancelled:" line; the history label returns "Cancelled" (never "Not yet started"); the planner marks entries ("— Cancelled", not hidden); the digest gained `DigestCancellation` rendering exactly `Cancelled: <tier> — <reason>`.
- **D3 — the repeatable acquisition loop.** New `EnsureSignablePoolUseCase` runs from the weekly handler **AFTER** `startDueTournaments` (placements first, then measure what is left signable — ordering pinned by a new handler test) and tops the pool up to `MIN_SIGNABLE_FREE_AGENTS` (25) using `PlayerRepository.countSignableFreeAgents()` — the exact claim predicate — generating across the band ranges. "You can always sign someone" is now a hard invariant.

**Pilot (real Postgres, throwaway copy `tennis_manager_agents_pilot2`, `WORLD_ID=agents`, the real `advance-world-day` handler at a 5s game day, `WORLD_TICK_PROFILE=1`), 6 scripted weeks:** signable never hit zero (start 871 → 144 → 28 → 695 → 1,052 → 961 → 1,148); every due week seeded completely (S2W1 28/28, W2 26/26, W3 23/23, W4 29/29, W5 23/23, W6 26/26, W7 23/23) and **all 995 tournaments scheduled before S2W1 — including the 48-draw never-started backlog and its 18 manager-entered draws — are now started**; no cancellation had to fire (the root fix removed the need; the path itself is proven by a real-Postgres route test: stuck draw cancelled → pool unblocked → claim 201). A controlled pre-fix run on the same world stayed at signable 0 after a week with 4 draws still unstarted. Weekly-rollover profile: pre-fix 19.5s (stuck, doing almost nothing) vs the fix's first rollover 47.4s (seeding/simulating the entire backlog) settling to 17-26s; `ensureFillOnlyPopulation` 9ms (vacuous, pre-fix) vs 289-728ms (actually sizing), plus new `ensureSignablePool` 167-416ms. Suites: domain 393, application 283, api 166, worker 13 — all green; `tsc --build --force` and `apps/web` typecheck clean.

## Season post-mortem pass (D1/D2/D4)

**D1 — same-event doubles was unenterable at the senior cap (a real bug, fixed).** `countSameBandEntriesForWeek` deduped by tournament, but the app-level pre-check in `RegisterEntrantUseCase`/`RegisterDoublesEntrantUseCase` counted the tournament BEING registered against itself, while the atomic guard (`DrizzleWeeklyEntryGuardAdapter`) already excluded it — so a senior at the cap of 1 holding a singles entry was refused the same event's doubles, making doubles strictly dominated (the 52-week agent season: 0 doubles entries in 208 decisions). The count function gained an `excludeTournamentId` parameter (plus a set-returning `sameBandEntryTournamentIdsForWeek`, so the route's `attachEntryInfo` display subtracts per row without re-reading the repository), both use cases pass `tournament.id`, and the modal's "N of cap" now mirrors enforcement exactly. UI: `EnterTournamentModal` states the cap fact ("singles and doubles at the SAME event is one tournament"), and `SinglesEntryPanel` offers "Also enter doubles at this event — no extra weekly entry" after a successful singles entry, with a roster select (partner included). Doubles points/prize deliberately untouched (verified realistic). Tests: application — singles→doubles and doubles→singles at the same event both pass, doubles at a DIFFERENT event is still refused at cap 1; real-Postgres atomic-guard case (a same-event doubles claim passes for a player already holding that event's singles entry, a different event is refused); api HTTP case proving the preview (0/1 on the entered event, 1/1 on a different one) and enforcement agree, and the different-event doubles POST 409s.

**D2 — titles are now tier-weighted in DISPLAY (the ladder and `creditFor` are deliberately untouched).** A raw title count is tier-blind (73 J30/J60 titles ≠ 29 titles including a major, yet "73 > 29"). New domain `TitleWeight.ts`: `titleWeightFor(tier)` = that tier's champion value read from the SAME `StandardRankingPointsTable` the sim awards from (one source of truth, no new constant), plus `summarizeTitleTiers`/`TitleTally`. `DrizzleTitleRepository.countByPlayers` is now a grouped-by-(player,tier) read returning count + weight + per-tier breakdown; the profile DTO gains `titleSummary` and the free-agent DTO gains `titleWeight`/`titlesByTier` alongside `titleCount`. The player-profile headline and the Scouting card render `titleSummaryLabel` (web `lib/titles.ts`), always showing count AND weight together ("73 titles · 4,380 pts · best J60" vs "29 titles · 3,350 pts · 1 Major"). Tests: `TitleWeight.test.ts` (pure weight + summary), the grouped repo integration case, HTTP DTO fields (a real major title round-trips `titleWeight: 2000`, `titlesByTier: { major: 1 }`), and label cases in `display-logic.spec.ts`.

**D4 — fatigue/form equilibrium pass (all constants PLACEHOLDER; the day-tick handler's system order untouched).** Fatigue recovery is now SELF-LIMITING: `fatigueRecoveredPerDay = FATIGUE_RECOVERY_PER_DAY (3) + fatigue × FATIGUE_RECOVERY_FRACTION (0.05)`, rounded — the formula lives in `packages/domain/src/player/FatiguePolicy.ts`, `Player.recoverFatigue` applies it, and `DrizzlePlayerRepository.recoverFatigueForAll` mirrors it in one SQL statement (`round((base + fatigue * fraction)::numeric)`); equivalence pinned against real Postgres across fatigue values. Form softened (`FORM_STALE_THRESHOLD` 30 → 40, `FORM_OUT_OF_BAND_PENALTY_PER_POINT` 0.3 → 0.15) and the inactivity penalty relaxed (`ManagerLadderPolicy` 0.85 → 0.95) so a rest week is a real option. Measured with the extended tool (3000 trials/bucket): a 5-match title run 91 → 18 end-of-week / **26 peak** (design predicted ~26); a 7-match major 97 → 63 at low stamina (predicted ~60); a 60-fatigue player recovers to ~24 in 7 idle days; the 6-match/week form equilibrium (31) is no longer stale. `docs/balance-tuning-report.md` carries the full before/after tables and states plainly that this deliberately shifts the rest pressure the earlier 3/day retune created. Tests: `FatiguePolicy.test.ts` equilibrium math, `Player.test.ts` recovery-with-fraction, an `AdvanceWorldWeekUseCase.test.ts` bulk==per-player case, form threshold boundaries (`FormModifier.test.ts`), and the ladder value. Suites: domain 404, application 287, api 169, worker 13 — all green; full `tsc --build --force` and `apps/web` typecheck clean.
