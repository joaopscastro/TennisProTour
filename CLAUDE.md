...
- `StatisticalMatchSimulator`'s scoring loop treats each simulated "point" as directly incrementing a game count, skipping real game-by-game/deuce structure. This is fine for validating the architecture, but is the single piece most worth a dedicated tuning + test-heavy pass before launch — it's the credibility core of the whole game; players will forgive UI roughness far more than a sim that "feels rigged."
